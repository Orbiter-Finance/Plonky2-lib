    ...
    InnerC::Hasher: AlgebraicHasher<F>,
{
    ...
    for i in pt.public_inputs.iter() {
        builder.register_public_input(*i);
    }
...
pub struct VerifierConfig {
    ...
    fri_constants_sigmas_cap_height: usize,
...
pub struct ProofForCircom {
    ...
    fri_constants_sigmas_cap: Vec<Vec<String>>,
...
>(
    ...
    let conf = VerifierConfig {
        ...
        fri_constants_sigmas_cap_height: verifier_only.constants_sigmas_cap.0.len(),
...
>(
    ...
    let mut proof_size: usize =
        (conf.num_wires_cap + conf.num_plonk_zs_partial_products_cap + conf.num_quotient_polys_cap)
            * conf.hash_size;
    let mut circuit_digest = vec!["0".to_string(); 4];
    let h = verifier_only.circuit_digest.to_vec();
    for i in 0..4 {
        circuit_digest[i] = h[i].to_canonical_u64().to_string();
    }
    proof_size += conf.hash_size;
    ...
    let mut fri_constants_sigmas_cap =
        vec![vec!["0".to_string(); 4]; conf.fri_commit_merkle_cap_height];

    for i in 0..conf.fri_constants_sigmas_cap_height {
        let h = verifier_only.constants_sigmas_cap.flatten();
        assert_eq!(h.len(), 4 * conf.fri_constants_sigmas_cap_height);
        for j in 0..4 {
            fri_constants_sigmas_cap[i][j] = h[i * 4 + j].to_canonical_u64().to_string();
        }
    }

    //add usize for fri_constants_sigmas_cap len:
    proof_size += std::mem::size_of::<usize>();
    let fri_constants_sigmas_cap_size = conf.fri_constants_sigmas_cap_height * conf.hash_size;
    proof_size += fri_constants_sigmas_cap_size;
    ...
    let verify_bytes = verifier_only.to_bytes().unwrap();

    assert_eq!(proof_bytes.len() + verify_bytes.len(), proof_size);
...
>(
    ...
    path.push("src/recursive_proof/circom_template/template_constants.circom");
    let mut constants = std::fs::read_to_string(path)
    ...
    // Load gate template
    let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    path.push("src/recursive_proof/circom_template/template_gates.circom");
    let mut gates_lib = std::fs::read_to_string(path)
        .expect("Something went wrong reading the file");
