...
INPUT_PATH=../test_data/proof.json
...
NODE_PARAMS="--trace-gc --trace-gc-ignore-scavenger --max-old-space-size=1568000 --initial-old-space-size=1568000  --no-incremental-marking --max-semi-space-size=1024 --initial-heap-size=1568000 --expose-gc"
...
start=$(date +%s)
cd ${CIRCUIT_NAME}_cpp && make -j$(nproc) && cd ..
end=$(date +%s)
echo "DONE ($((end - start))s)"

echo "****cpp WITNESS GENERATION****"
start=$(date +%s)
